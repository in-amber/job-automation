...
- [ ] `submit_policy.auto_submit_allowed` is `true` AND `submit_policy.human_approval_required` is `false`. If either fails, stop here and create a review request instead.
...
If `submit_policy.human_approval_required: true` OR `submit_policy.auto_submit_allowed: false`:
