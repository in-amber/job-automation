...
- Auto-submit is allowed when `job-automation/config/runtime.json: auto_submit_linkedin_easy_apply` is true.
...
- Submit only if `job-automation/config/runtime.json: auto_submit_greenhouse` is true; otherwise stop at submit per `submit_policy`.
...
- Submit only if `job-automation/config/runtime.json: auto_submit_workday` is true.
- If field mapping remains broken after retry limits, create an intervention report and move on.

## Other / unknown (`other`)

- Auto-submit is allowed when `job-automation/config/runtime.json: auto_submit_other` is true; the packet's `submit_policy` is the source of truth at runtime.
